...
//            echo BASE_URL;die;
        ...
        // Main query to actually get the data
        $data = self::sql_exec($db, $bindings, "SELECT `" . implode("`, `", self::pluck($columns, 'db')) . "` FROM `$table` WHERE uploaded_file_volroot=0 AND rmsa_employee_users_id='$emp_rmsa_user_id'  $order $limit"
        );
        // Data set length after filtering
        $resFilterLength = self::sql_exec($db, $bindings, "SELECT COUNT(`{$primaryKey}`)
			 FROM   `$table`
        ...
        if (!empty($result)) {
            foreach ($result as $row) {
                ...
                if ($row['uploaded_file_hasvol'] == "YES") {
                    ...
                    $dataChild = self::sql_exec($db, $bindings, "SELECT `" . implode("`, `", self::pluck($columns, 'db')) . "`
                                     FROM `$table`
...
//                print_r($resData);die;
    ...
    static function student_file_list($request, $conn, $table, $primaryKey, $columns, $where_custom = '') {
        ...
        // Main query to actually get the data
        $data = self::sql_exec($db, $bindings, "SELECT `" . implode("`, `", self::pluck($columns, 'db')) . "` FROM `$table` WHERE uploaded_file_volroot=0  $order $limit"
        );
        // Data set length after filtering
        $resFilterLength = self::sql_exec($db, $bindings, "SELECT COUNT(`{$primaryKey}`)
			 FROM   `$table`
...
//                             </td>
                ...
                $reviews = self::sql_exec($db, "SELECT AVG(rmsa_file_rating) as overall_rating FROM rmsa_file_reviews
                ...
                </br> <span>$star</span><a href='/rmsa/file-reviews'>View Reviews</a></td>";
