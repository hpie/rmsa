...
//        echo 'hi';die;
        ...
        static function teacher_student_list ( $request, $conn, $table, $primaryKey, $columns,$where_custom = '')
	{
		$bindings = array();
		$db = self::db( $conn );
		// Build the SQL query string from the request
...
//        echo 'hi';die;
        ...
        if(!empty($result)){
            foreach ($result as $row){
                ...
                $row['rmsa_user_status'] = "<button type='button' data-id='".$row['rmsa_user_id']."' data-status = '".$isactive."' title='".$title."' class='".$class." btn-xs'>".$text."</button>";
                $row['rmsa_user_edit'] = "<a href='".BASE_URL."/employee-update-student-profile/$stud_id' class='btn btn-xs btn-warning'>Edit  <i class='fa fa-pencil'></i></a>";                
                $row['rmsa_user_edit'] .= "<br><a href='".BASE_URL."/teacher-student-attempted-quiz-list/$stud_id' class='btn btn-xs btn-warning'>Quiz Result<i class='fa fa-eye'></i></a>";
                $row['index']='';
                array_push($resData, $row); 
            }
        }
...
//        print_r($resData);die;
		...
		return array(
          ...
          static function wrong_login_list ( $request, $conn, $table, $primaryKey, $columns,$where_custom = '')
	{
		$bindings = array();
		$db = self::db( $conn );
		// Build the SQL query string from the request
...
//                echo $sql;die;
        ...
        static function rmsa_teachers_list ( $request, $conn, $table, $primaryKey, $columns,$where_custom = '')
	{
        ...
        static function rmsa_school_wise_count_student ( $request, $conn, $table, $primaryKey, $columns,$where_custom = '')
	{
		$bindings = array();
		$db = self::db( $conn );
		// Build the SQL query string from the request
        ...
        if(!empty($result)){
            foreach ($result as $row){
                $school_id=$row['rmsa_school_id'];
                
                $student_total_count = self::sql_exec($db,"SELECT COUNT(*) as total_Student FROM rmsa_student_users WHERE rmsa_school_id = '{$school_id}'");                                
                $row['index']='';
                $row['total_Student']=$student_total_count[0]['total_Student'];
                array_push($resData, $row); 
            }
        }
		return array(
         ...
         static function rmsa_district_wise_count_student ( $request, $conn, $table, $primaryKey, $columns,$where_custom = '')
	{
		$bindings = array();
		$db = self::db( $conn );
		// Build the SQL query string from the request
        ...
        if(!empty($result)){
            foreach ($result as $row){
                $dist_id=$row['rmsa_district_id'];                
                $student_total_count = self::sql_exec($db,"SELECT COUNT(*) as total_Student FROM rmsa_student_users WHERE rmsa_district_id = '{$dist_id}'");                                
                $row['index']='';
                $row['total_Student']=$student_total_count[0]['total_Student'];
                array_push($resData, $row); 
            }
		...
		return array(
         ...
         static function rmsa_block_wise_count_student ( $request, $conn, $table, $primaryKey, $columns,$where_custom = '')
	{
		$bindings = array();
		$db = self::db( $conn );
		// Build the SQL query string from the request
        ...
        if(!empty($result)){
            foreach ($result as $row){
                $block_id=$row['rmsa_block_id'];                
                $student_total_count = self::sql_exec($db,"SELECT COUNT(*) as total_Student FROM rmsa_student_users WHERE rmsa_block_id = '{$block_id}'");                                
                $row['index']='';
                $row['total_Student']=$student_total_count[0]['total_Student'];
                array_push($resData, $row); 
            }
		...
		$recordsTotal = $resTotalLength[0][0];                
                ...
                if(!empty($result)){                    
                    foreach ($result as $row){                           
                        ...
                        $row['ext']="<td style='padding: 0px 0px;' class='tooltip1'><center><a href='".$link_str."' class='viewFile'><img src='".IMG_URL."/assets/front/fileupload/img/file-icon/icon/".$row['uploaded_file_type'].".png'><br>".$row['uploaded_file_title']."</a>
                        ...
                        if($row['uploaded_file_hasvol']=="YES"){
                            ...
                            foreach ($resultChild as $rowChild){
                                ...
                                if($i<=1){                                
                                    $strTd.="<td style='padding: 0px 0px;' class='tooltip1'><center><a href='".$link_str_child."' class='viewFile'><img src='".IMG_URL."/assets/front/fileupload/img/file-icon/icon/".$rowChild['uploaded_file_type'].".png' ><br>".$rowChild['uploaded_file_title']."</a>
                                    ...
                                    $strTdAmodel.="<td style='padding: 0px 0px;' class='tooltip1'><center><a href='".$link_str_child."' class='viewFile'><img src='".IMG_URL."/assets/front/fileupload/img/file-icon/icon/".$rowChild['uploaded_file_type'].".png' ><br>".$rowChild['uploaded_file_title']."</a>
                                             <br><span style='font-size:10px' class='tooltiptext'>Hit count <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$rowChild['uploaded_file_viewcount']."<br>Student view <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$total_student_view_child."</span></center></td>";                                                                       
                                $i=$i+1;                               
                            }
                            $str.=$strTd."</tr></table>";
                ...
                if(!empty($result)){                    
                    foreach ($result as $row){                                                                                                                                      
                        ...
                        if($attendSts == 0){
                            ...
                            $row['ext']="<td style='padding: 0px 0px;' class='tooltip1'><center><a href='".$link_str."'><img src='".IMG_URL."/assets/front/fileupload/img/file-icon/icon/".$row['uploaded_file_type'].".png' ><br>".$row['uploaded_file_title']."</a>
                                <br><span style='font-size:10px' class='tooltiptext'>Hit count <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$row['uploaded_file_viewcount']."<br>Student view <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$total_student_view."</span></center></td>";
                        }
                        else{
                            $row['ext']="<td style='padding: 0px 0px;' class='tooltip1'><center><a class='view_count' data-id='".$row['rmsa_uploaded_file_id']."' href='".$link_str."'><img src='".IMG_URL."/assets/front/fileupload/img/file-icon/icon/".$row['uploaded_file_type'].".png' ><br>".$row['uploaded_file_title']."</a>
                                    <br><span style='font-size:10px' class='tooltiptext'>Hit count <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$row['uploaded_file_viewcount']."<br>Student view <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$total_student_view."</span></center></td>";
                        }
...
//              print_r($resData);die;
		...
		return array(
         ...
         static function tec_stud_attempted_quiz_list ($request, $conn, $table, $primaryKey, $columns,$where_custom = '',$st_rmsa_user_id)
	{                 
		$bindings = array();
		$db = self::db( $conn );
...
//                                     <br><span style='font-size:10px' class='tooltiptext'>Hit count <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$row['uploaded_file_viewcount']."<br>Student view <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$total_student_view."</span></center></td>";
                        ...
                        $row['action'] = "<a href='".BASE_URL."/tec-my-quiz-attempt-result/$quiz_id/$st_rmsa_user_id' class='btn btn-xs btn-warning'>Quiz Score<i class='fa fa-eye'></i></a>";
...
//              print_r($resData);die;
        ...
        static function  tec_my_quiz_result($request, $conn, $table, $primaryKey, $columns,$where_custom = '',$quiz_pass_score)
	{                         
		$bindings = array();
		$db = self::db( $conn );
...
//                                echo $attendStsChild;
                        ...
                        if($attendSts==0){
                                ...
                                if($i<=1){                                                                
                                        $strTd.="<td style='padding: 0px 0px;' class='tooltip1'><center><a href='".$link_str_child."'><img src='".IMG_URL."/assets/front/fileupload/img/file-icon/icon/".$rowChild['uploaded_file_type'].".png' ><br>".$rowChild['uploaded_file_title']."</a>
                                             <br><span style='font-size:10px' class='tooltiptext'>Hit count <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$rowChild['uploaded_file_viewcount']."<br>Student view <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$total_student_view_child."</span></center></td>";                                    
                                }
                                $strTdAmodel.="<td style='padding: 0px 0px;' class='tooltip1'><center><a href='".$link_str_child."'><img src='".IMG_URL."/assets/front/fileupload/img/file-icon/icon/".$rowChild['uploaded_file_type'].".png' ><br>".$rowChild['uploaded_file_title']."</a>
                                 <br><span style='font-size:10px' class='tooltiptext'>Hit count <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$rowChild['uploaded_file_viewcount']."<br>Student view <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$total_student_view_child."</span></center></td>";
                        }else{                            
                            if($i<=1){                            
                                if($attendStsChild == 0){
                                ...
                                $strTd.="<td style='padding: 0px 0px;' class='tooltip1'><center><a class='view_count' data-id='".$rowChild['rmsa_uploaded_file_id']."' href='".$link_str_child."'><img src='".IMG_URL."/assets/front/fileupload/img/file-icon/icon/".$rowChild['uploaded_file_type'].".png' ><br>".$rowChild['uploaded_file_title']."</a>
                                         <br><span style='font-size:10px' class='tooltiptext'>Hit count <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$rowChild['uploaded_file_viewcount']."<br>Student view <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$total_student_view_child."</span></center></td>";
                                }
                            }
                            if($attendStsChild == 0){                                
                            ...
                            $strTdAmodel.="<td style='padding: 0px 0px;' class='tooltip1'><center><a class='view_count' data-id='".$rowChild['rmsa_uploaded_file_id']."' href='".$link_str_child."'><img src='".IMG_URL."/assets/front/fileupload/img/file-icon/icon/".$rowChild['uploaded_file_type'].".png' ><br>".$rowChild['uploaded_file_title']."</a>
                                     <br><span style='font-size:10px' class='tooltiptext'>Hit count <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$rowChild['uploaded_file_viewcount']."<br>Student view <i class=\"fa fa-eye\" aria-hidden=\"true\"></i> ".$total_student_view_child."</span></center></td>";
                            }
                        }
                        $i=$i+1; 
